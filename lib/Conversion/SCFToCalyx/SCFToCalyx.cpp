...
bool isDefinedOutsideRegion(Value value, Region *targetRegion) {
  ...
  Operation *parentOp = definingOp->getParentOp();
  while (parentOp) {
    if (parentOp->getParentRegion() == targetRegion) {
      return false;
    }
      ...
      for (size_t idx = 0; idx < accessIndices.size(); idx++) { 
        ...
        scfParOp.walk([&](mlir::Operation *innerOp) {
          ...
          for (auto operand : innerOp->getOperands()) {
            if (llvm::find(scfParOp.getInductionVars(), operand)) {
            //if (isa<BlockArgument>(operand) && operand.getParentBlock() == scfParOp.getBody()) {
              operand.dump();
              // meaning it's one of the induction variables
        ...
        for (auto &innerOp : scfParOp.getOps()) {
          ...
          innerOp.walk([&](Operation *nestedOp) {
            for (auto nestedOperand : nestedOp->getOperands()) {
              if ((isa<BlockArgument>(nestedOperand) && nestedOperand.getParentBlock() == scfParOp.getBody())                 || isDefinedOutsideRegion(nestedOperand, &scfParOp->getRegion(0))) {
